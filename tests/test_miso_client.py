...
def test_write_and_flush():
    ...
    client = MisoWriter(api_server='https://test.com', api_key='secret')
    ...
    client.session.post.assert_called_once_with(
        'https://test.com/v1/products?api_key=secret', json={'data': [product] * 100}
    )
    client.session.post = MagicMock()
    user = {'user_id': 'test'}
    for _ in range(100):
    ...
    client.session.post.assert_called_once_with(
        'https://test.com/v1/users?api_key=secret', json={'data': [user] * 100}
    )
